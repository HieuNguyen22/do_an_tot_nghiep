...
import 'package:app_work_log/modules/overtime_request/provider/overtime_request_provider.dart';
...
class OvertimeRequestController extends GetxController {
  ...
  RxString dateOT = DateFormat("yyyy/MM/dd").format(DateTime.now()).obs;
  ...
  submit() {
    ...
    if (isValidManager.value &&
      ...
      String timeStartRequest = dateOT.value + " " + timeStart.value;
      String timeEndRequest = dateOT.value + " " + timeEnd.value;
  ...
  Future<void> callApiManagerOfCompany() async {
    var result =
        await OvertimeRequestProvider().getListManagerofCompanyUserLogin();
  ...
  void callApiSendLeaveRequestProvider(
    ...
    bool result = await OvertimeRequestProvider().sendOvertimeRequest(
        companyId: companyId,
        userId: userId,
  ...
  validateDate() {
    if ((dateOT.value.isEmpty || dateOT.value == "----/--/--") ||
        (dateOT.value.isEmpty || dateOT.value == "----/--/--")) {
  ...
  getDateRange(List<DateTime?> dateRange) {
    ...
    if (dateRange[0] != null) {
      dateOT.value = format.format(dateRange[0]!);
    }

    if (dateRange[1] != null) {
      dateOT.value = format.format(dateRange[1]!);
    }
  }
