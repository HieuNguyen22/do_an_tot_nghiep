...
import 'package:attendance_fast/modules/leave_request_manage/widgets/confirm_leave_request_view.dart';
import 'package:attendance_fast/modules/leave_request_manage/widgets/detail_leave_request_view.dart';
import 'package:attendance_fast/themes/app_color.dart';
import 'package:attendance_fast/themes/style_text.dart';
...
class ItemLeaveRequest extends GetView<LeaveRequestManageController> {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        if (item.statusCode == Numeral.STATUS_CODE_PENDING) {
          controller.isConfirmLeaveRequest.value = false;
          controller.textReasonController.value.text = "";
          Get.dialog(ConfirmLeaveRequestView(item: item,),);
        } else {
          Get.dialog(DetailLeaveRequestView(item: item,),);
        }
      },
      child: Column(
        children: [
          ...
          Container(
            margin:
                EdgeInsets.only(top: Get.height * 0.005, left: Numeral.WIDTH_SCREEN * 0.1),
            child: Row(
              children: [
                Expanded(
                  child: BaseText(
